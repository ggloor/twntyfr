...
scale.factor=30000 #specific to VIRGO dataset
# Generalized
scale.factor<-abs(max(d.pca$x[,2]))/abs(max(d.pca$rotation[,2]))*2

p<-ggplot() +
	geom_point(data = d.pca$rotation, aes(x=PC1, y=PC2), alpha = 1/20) +
	geom_point(data = d2, aes(x=PC1/scale.factor, y=PC2/scale.factor, colour = nugent_status)) +
	geom_text(data = d2, aes(x=PC1/scale.factor, y=PC2/scale.factor, label = rownames(d2), colour = nugent_status), hjust = 0, nudge_x = 0.0005, show.legend = FALSE
)
p + ggtitle("PCA") +
  xlab(xlab) + ylab(ylab) +
  theme_bw()
