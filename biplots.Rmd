...
biplot(d.pcx,col=c("black", rgb(0,0,0,0.1)),cex=c(0.8,0.05),
var.axes=F, scale=0)
# this is phi at a 0.03 cutoff
g.df <- read.table("data/g.df.txt", header=T, row.names=1)
...
#print("")
points(d.pcx$rotation[nms,][,1],d.pcx$rotation[nms,][,2], col=colours[i],
    pch=19, cex=0.3)
}
```
```{r biplot_phi5}
...
# this is phi at a 0.05 cutoff
g.df5 <- read.table("data/g.df0.05.txt", header=T, row.names=1)

big5 <- g.df5[which(g.df5$cluster.size >= 2),]
colnames(big5) <- colnames(g.df5)

lev <- factor(big5$cluster)
for(i in as.numeric(levels(lev))){
nms <- rownames(big5)[big5$cluster==i]
#print(rownames(big5)[big5$cluster==i])
#print("")
points(d.pcx$rotation[nms,][,1],d.pcx$rotation[nms,][,2], col=colours[i],
    pch=19, cex=0.3)
}
