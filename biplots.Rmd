...
e.min.sym.phi <- propr.phisym(t(sapply(getMonteCarloInstances(e.x), function(y){y[,1]})))
...
for(i in 2:numMCInstances(e.x)){
	#print(i)
	e.min.sym.phi <- propr.phisym(t(sapply(getMonteCarloInstances(e.x), function(y){y[,i]})))
	e.min.phi.min <- e.min.phi.min + lt.row.min(e.min.sym.phi)
}
# get the expected value
e.min.phi.min <- e.min.phi.min /numMCInstances(e.x)
##### Done Bayesian stuff
