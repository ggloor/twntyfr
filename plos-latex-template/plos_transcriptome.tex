...
A final complication is that the measurement error of compositional data is different than that expected for non-compositional data being non-linearly related to the read depth \cite{fernandes:2013}. In fact, the measurement error is inversely and 
%A second major problem with compositional data is that of permutation invariance. This means that the same answer should be obtained regardless of how the data is arranged. As an example, the current gold-standard for mRNA quantition is real-time PCR where the RNA species of interest is expressed as a ratio to the abundance of an internal standard --- usually a housekeeping gene. It is well-known that he choice of the housekeeping gene has a significant effect on the outcome \cite{kozera:2013}, making  this approach  not  permutation invariant. 

It has been argued that that microbial communities are best though of as ensembles of genes since the genetic makeup of individual bacterial genomes is rather fluid. 
