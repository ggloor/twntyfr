...
Traditional statistical approaches, while widely used, are inappropriate when examining compositional data since these datasets contain information only on the ratios between the components (REFS).  For this reason, we examined the correlation of functions using the $\phi$-metric that assesses the stability of the ratios across samples of all possible pairs of transcripts (REFS). Samples that have stable ratios are said to be compositionally associated and have a value of $phi$ near 0.

Figure \ref{phi} shows a graphical depiction of $\phi$ groupings that 
